...
export default function CanvasGraphicDetails({ activeWarehouse }) {
  ...
  function drawCompBase(data) {
    ...
    for (let i = 0; i < monthGraphNames.length; ++i) {
      monthGraphNames[i] = monthGraphNames[i]
        .toString()
        .substr(3, 5)
        .trim();
    }
    ...
    return graphViewFlag
      ? drawCompData(normalizeNum)
      : drawLtrGraph(normalizeNum);
  }
  ...
  const toggleErr = error => {
    ...
    if (
      ...
      const httpError = new Error(
        "can't retrieve data - API resource isn't exsist or remote json file unsturctured"
      );
  ...
  const [
    graphHeadLine,
    mainCanvasStyle,
    secondaryCanvasStyle,
    altentiveCanvasStyle
  ] = [
    { fontFamily: "Comic", color: "orange" },
    {
      backgroundColor: "lavender",
      transform: "skewX(8deg) rotateX(180deg) rotateY(180deg)"
    },
  ...
  return (
    <div>
        ...
        <div>
          ...
          {graphViewFlag ? (
            <>
              ...
              ></canvas>
              <span style={quantitySpan}>
                Stock quantity <br />
                <b className="text-danger">{units}</b> per unit
              </span>
            ...
            <>
              ...
              ></canvas>
              <span style={quantitySpan}>
                Stock quantity | (<b className="text-danger">{units}</b> per
                unit)
              </span>
      ...
      >
        {graphViewFlag
          ? "Show Regular Graph Direction"
          : "Show Right To Left Graph"}
      </Button>
      {!err && (
        <Button
          outline
